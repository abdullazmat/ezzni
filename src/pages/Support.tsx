...
const MOCK_COMPLAINTS: ComplaintDetail[] = [
  {
    ...
    description: 'During my recent trip, the driver was consistently rude and made multiple inappropriate comments...',
    user: {
    ...
    messages: [
      { id: 'm1', sender: 'user', text: 'Hello, I have a question about my ticket COMP001.', time: '14:30' }
    ]
  },
  {
    id: '2',
    ...
    category: 'Delivery',
    description: 'The delivery package was damaged upon arrival and the driver refused to acknowledge it.',
    user: {
      name: 'Sarah Mohamed El-Fassi',
      id: 'R-00045',
      type: 'driver',
    ...
    assignedTo: 'Ali',
    messages: [
      { id: 'm2', sender: 'user', text: 'Where is my delivery?', time: '09:45' },
      { id: 'm3', sender: 'admin', text: 'We are looking into it.', time: '10:00' }
    ]
  },
  {
    id: '3',
...
export const Support = () => {
  ...
  const stats = useMemo(() => {
    return {
      ...
      open: complaints.filter(c => c.status === 'New' || c.status === 'Pending').length,
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case 'New': return { backgroundColor: '#FEF2F2', color: '#EF4444' };
      case 'In Progress': return { backgroundColor: '#EFF6FF', color: '#3B82F6' };
    ...
    <>
      <div className="stats-grid">
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span className="stat-value" style={{ color: filterStats === 'open' ? 'white' : 'black' }}>00{stats.open}</span>
            <div className="stat-arrow" style={{ backgroundColor: filterStats === 'open' ? 'black' : '#f3f4f6', color: filterStats === 'open' ? 'white' : 'black' }}>
               <ArrowUpRight size={20} />
            </div>
          </div>
        </div>
        ...
        <table className="support-table">
          ...
          <tbody>
            {filteredTickets.length > 0 ? filteredTickets.map((item) => (
              ...
              <tr>
                <td colSpan={7} style={{ textAlign: 'center', padding: '3rem', color: '#9CA3AF' }}>No complaints found matching filters</td>
              </tr>
            )}
          </tbody>
        </table>
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div className="flex justify-between" style={{ marginBottom: '4px' }}>
                  <span style={{ fontWeight: 'bold', fontSize: '14px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{complaint.user.name}</span>
                  <span style={{ fontSize: '10px', color: '#9CA3AF', flexShrink: 0 }}>2m ago</span>
                </div>
                <div className="flex gap-2" style={{ marginBottom: '4px' }}>
                   <span style={{ fontSize: '10px', backgroundColor: '#EFF6FF', color: '#3B82F6', padding: '2px 6px', borderRadius: '4px' }}>{complaint.user.type}</span>
                   <span style={{ fontSize: '10px', color: '#9CA3AF' }}>{complaint.user.id}</span>
                </div>
      ...
      <div style={{ flex: 1, backgroundColor: 'white', borderRadius: '24px', border: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {activeChat ? (
             <div className="flex h-full" style={{ flexDirection: 'column' }}>
                ...
                {/* Messages Area */}
                <div className="flex" style={{ flex: 1, padding: '1.5rem', overflowY: 'auto', backgroundColor: '#f9fafb', flexDirection: 'column', gap: '1.5rem' }}>
                    ...
                    {activeChat.messages.map(msg => (
                      <div key={msg.id} style={{ alignSelf: msg.sender === 'user' ? 'flex-start' : 'flex-end', maxWidth: '75%' }}>
                          <div style={{ 
                            ...
                            padding: '1rem', 
                            borderRadius: '16px', 
                            borderBottomLeftRadius: msg.sender === 'user' ? 0 : '16px',
                          ...
                          <span style={{ fontSize: '10px', color: '#9CA3AF', marginTop: '6px', textAlign: msg.sender === 'user' ? 'left' : 'right', display: 'block' }}>{msg.time}</span>
                      </div>
                    ))}
                    <div ref={chatEndRef} />
                </div>
                ...
                <div style={{ padding: '1.25rem', borderTop: '1px solid #f3f4f6' }}>
                    <form className="flex items-center gap-2" style={{ backgroundColor: '#F3F4F6', padding: '0.5rem 1rem', borderRadius: '30px' }} onSubmit={handleSendChatMessage}>
                        ...
                        <input 
                            ...
                            style={{ flex: 1, border: 'none', background: 'none', outline: 'none', padding: '0.6rem 0', fontSize: '14px' }}
                        />
                        <button type="button" style={{ border: 'none', background: 'none', color: '#9CA3AF', cursor: 'pointer', padding: '0.4rem' }}>
                            <Smile size={20} />
                        </button>
    ...
    <div className="main-content" style={{ padding: '2rem', backgroundColor: '#f8fafc', height: '100%', overflowY: 'auto' }}>
      <div className="page-header" style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
            ...
            <button className="export-btn" style={{ padding: '12px 24px', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '10px', borderRadius: '14px' }}>
                <Download size={20} />
                Export Report
            </button>
        </div>
      ...
      {activeTab === 'Support Tickets' ? renderTicketsTab() : renderLiveChatTab()}
