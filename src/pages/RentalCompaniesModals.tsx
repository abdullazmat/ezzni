...
export const VehicleDetailModal = ({ vehicle, onClose, onApprove, onReject }: {
    ...
    const images = carImageSets[vehicle.name] || defaultImages;
            ...
            <div onClick={e => e.stopPropagation()} style={{ backgroundColor: 'white', borderRadius: '24px', width: '100%', maxWidth: '680px', maxHeight: '90vh', overflowY: 'auto', padding: '32px', position: 'relative' }}>
                ...
                <button 
                    ...
                    style={{ 
                        ...
                        borderRadius: '50%', border: 'none', backgroundColor: '#f3f4f6', 
                        cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: '18px', color: '#374151', transition: 'all 0.2s', zIndex: 10
                    }}
                    onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#e5e7eb'; e.currentTarget.style.transform = 'scale(1.1)'; }}
                        ...
                        <div>
                            ...
                            {isEditing ? (
                                <input value={editData.year} onChange={e => setEditData({...editData, year: e.target.value})}
                                    style={{ fontWeight: 'bold', fontSize: '15px', border: '1px solid #d1d5db', borderRadius: '6px', padding: '4px 6px', width: '100%', outline: 'none', boxSizing: 'border-box' }} />
                            ) : (
                        ...
                        <div>
                            ...
                            {isEditing ? (
                                <select value={editData.transmission} onChange={e => setEditData({...editData, transmission: e.target.value})}
                                    style={{ fontWeight: 'bold', fontSize: '14px', border: '1px solid #d1d5db', borderRadius: '6px', padding: '4px 6px', width: '100%', outline: 'none', cursor: 'pointer' }}>
                                    <option>Automatic</option><option>Manual</option>
                                </select>
                            ) : (
                                <div style={{ fontWeight: 'bold', fontSize: '15px' }}>{vehicle.transmission}</div>
                            )}
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            {isEditing ? (
                                <select value={editData.fuel} onChange={e => setEditData({...editData, fuel: e.target.value})}
                                    style={{ fontWeight: 'bold', fontSize: '14px', border: '1px solid #d1d5db', borderRadius: '6px', padding: '4px 6px', width: '100%', outline: 'none', cursor: 'pointer' }}>
                                    <option>Petrol</option><option>Diesel</option><option>Gas</option><option>Electric</option><option>Hybrid</option>
                                </select>
                            ) : (
                ...
                <div style={{ backgroundColor: '#f0fdf4', borderRadius: '12px', padding: '16px', marginBottom: '28px', border: '1px solid #dcfce7' }}>
                    {isEditing ? (
                        ...
                        <p style={{ fontSize: '13px', color: '#374151', margin: 0, lineHeight: '1.6' }}>{vehicle.description}</p>
                    )}
                </div>
                ...
                {isEditing ? (
                    ...
                    <div style={{ display: 'flex', gap: '12px' }}>
                        ...
                        <button onClick={onReject}
                            style={{ flex: 1, padding: '14px', borderRadius: '32px', border: 'none', backgroundColor: '#dc2626', color: 'white', fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', transition: 'all 0.2s' }}
...
export const AddVehicleModal = ({ onClose, onAdd }: { onClose: () => void; onAdd: (vehicle: Omit<VehicleListing, 'id' | 'companyLogo' | 'submittedDate' | 'status'>) => void }) => {
    ...
    return (
        ...
        >
            <div onClick={e => e.stopPropagation()} style={{ backgroundColor: 'white', borderRadius: '24px', width: '100%', maxWidth: '600px', maxHeight: '90vh', overflowY: 'auto', padding: '32px', position: 'relative' }}>
                ...
                <h2 style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: '24px', textAlign: 'center' }}>Add New Vehicle</h2>

                <div style={{ display: 'grid', gap: '16px' }}>
                    <div>
                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 'bold' }}>Vehicle Model</label>
                        <select 
                            value={formData.name} 
                            onChange={e => setFormData({...formData, name: e.target.value})}
                            style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #d1d5db', outline: 'none' }}
                        >
                    ...
                    <div>
                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 'bold' }}>Description</label>
                        <textarea 
                            value={formData.description} 
                            onChange={e => setFormData({...formData, description: e.target.value})}
                            style={{ width: '100%', height: '80px', padding: '12px', borderRadius: '12px', border: '1px solid #d1d5db', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' }} 
                        />
                    </div>
