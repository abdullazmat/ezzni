...
class RideSocketService {
  ...
  cancelRide(rideRequestId: number, reason?: string) {
    if (this.rideNamespace) {
      this.rideNamespace.emit('passenger:cancelRide', { rideRequestId, reason });
    }
  }

  // --- Driver Actions ---

  goOnline(driverInfo: { id: number; name: string; phone: string; imageUrl?: string | null }) {
    if (this.rideNamespace) {
  ...
  onRideReceived(callback: (data: { message: string; rideRequestId: number; timestamp: string }) => void) {
  ...
  onRideCompleted(callback: (data: { rideRequestId: number; finalPrice: number }) => void) {
    this.rideNamespace?.on('ride:completed', callback);
  }

  onRideCancelled(callback: (data: { rideRequestId: number; message: string }) => void) {
  ...
  onDriverRideAccepted(callback: (data: { success: boolean; ride?: any; message?: string }) => void) {
